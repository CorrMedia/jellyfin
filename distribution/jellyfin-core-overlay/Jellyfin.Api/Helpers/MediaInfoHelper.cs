...
public class MediaInfoHelper
{
    ...
    private static void ApplyEdlHlsDelivery(StreamInfo streamInfo, MediaSourceInfo mediaSource, DeviceProfile profile)
    {
        ...
        if (hlsProfile is not null)
        {
            var container = hlsProfile.Container;
            if (!string.IsNullOrEmpty(container))
            {
                streamInfo.Container = container.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)[0];
            }

            if (hlsProfile.SegmentLength > 0)
            {
                streamInfo.SegmentLength = hlsProfile.SegmentLength;
            }

            if (hlsProfile.MinSegments > 0)
            {
                streamInfo.MinSegments = hlsProfile.MinSegments;
            }
        }
        ...
        streamInfo.SegmentLength ??= 6;
        streamInfo.MinSegments ??= 1;
