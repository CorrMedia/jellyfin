...
public class DynamicHlsController : BaseJellyfinApiController
{
    ...
    private readonly IEnumerable<MediaBrowser.Controller.MediaEncoding.ISessionMuteRangeLoader> _muteRangeLoaders;
    ...
    public DynamicHlsController(
        ...
        IEnumerable<MediaBrowser.Controller.MediaEncoding.ISessionMuteRangeLoader>? muteRangeLoaders = null)
    {
        ...
        _muteRangeLoaders = muteRangeLoaders ?? Array.Empty<MediaBrowser.Controller.MediaEncoding.ISessionMuteRangeLoader>();
    ...
    private string GetAudioArguments(StreamState state)
    {
        ...
        if (!state.IsOutputVideo)
        {
            ...
            if (EncodingHelper.IsCopyCodec(audioCodec) && !_encodingHelper.HasSessionAudioFilter(state, _encodingOptions))
            {
            ...
            return audioTranscodeParams + _encodingHelper.GetAudioFilterParam(state, _encodingOptions);
        }
    ...
    private async Task ApplyEdlTranscodeFlagsAsync(StreamingRequestDto streamingRequest, string? itemId, CancellationToken cancellationToken)
    {
        ...
        var hasMute = _encodingHelper.HasSessionAudioFilterForRequest(playSessionId, deviceId);
        if (hasMute || hasEditGraph)
        {
        ...
        if (hasEditGraph)
        {
            ...
            // Keep StartTimeTicks for mid-stream HLS restart; graph truncates to edited seek.
        }
    }
    ...
    private async Task<bool> SessionHasMuteRangesAsync(string? playSessionId, string? deviceId, string? itemId, CancellationToken cancellationToken)
    {
        ...
        return _encodingHelper.HasSessionAudioFilterForRequest(playSessionId ?? string.Empty, deviceId ?? string.Empty)
            || _encodingHelper.HasSessionEditGraphForRequest(playSessionId ?? string.Empty, deviceId ?? string.Empty);
    }
