...
public class PackageController : BaseJellyfinApiController
{
        ...
        [FromQuery] Guid? assemblyGuid)
    {
        ...
        // Sideloaded plugins are not in the official catalog; jellyfin-web treats a 404 as
        // "An error occurred while getting the plugin details from the repository."
        result ??= TryPackageInfoFromInstalled(name, assemblyGuid);
    ...
    private PackageInfo? TryPackageInfoFromInstalled(string name, Guid? assemblyGuid)
    {
        LocalPlugin? plugin = null;
        if (assemblyGuid.HasValue && !assemblyGuid.Value.IsEmpty())
        {
            plugin = _pluginManager.GetPlugin(assemblyGuid.Value);
        }

        plugin ??= _pluginManager.Plugins.FirstOrDefault(p =>
        ...
        var description = string.IsNullOrEmpty(info.Description) ? manifest.Description : info.Description;
        var overview = string.IsNullOrEmpty(manifest.Overview) ? description : manifest.Overview;
        ...
        return new PackageInfo
        {
            ...
            [
                new VersionInfo
                {
                    ...
                    Changelog = string.IsNullOrEmpty(manifest.Changelog) ? null : manifest.Changelog,
                    TargetAbi = string.IsNullOrEmpty(manifest.TargetAbi) ? null : manifest.TargetAbi,
                    Timestamp = manifest.Timestamp == default
                        ? null
                        : manifest.Timestamp.ToString("O", CultureInfo.InvariantCulture),
