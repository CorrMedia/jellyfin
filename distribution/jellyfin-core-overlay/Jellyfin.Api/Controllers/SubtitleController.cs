...
public class SubtitleController : BaseJellyfinApiController
{
    ...
    private readonly ISessionSubtitleCueRewriter _subtitleCueRewriter;
    ...
    /// <param name="subtitleCueRewriter">Instance of <see cref="ISessionSubtitleCueRewriter"/> interface.</param>
    ...
    public SubtitleController(
        ...
        ISessionSubtitleCueRewriter subtitleCueRewriter,
        ILogger<SubtitleController> logger)
    {
        ...
        _subtitleCueRewriter = subtitleCueRewriter;
        _logger = logger;
    }
    ...
    private async Task<Stream> EncodeSubtitles(
        Guid id,
        ...
        bool copyTimestamps)
    {
        ...
        var itemId = id.ToString("N", CultureInfo.InvariantCulture);

        if (_subtitleCueRewriter.NeedsRewrite(itemId, format))
        {
            // Fetch the full original-timeline track, then remap and window on the edited clock.
            // Windowing first against original cue times would drop/shift the wrong events.
            var original = await _subtitleEncoder.GetSubtitles(
                item,
                mediaSourceId,
                index,
                format,
            ...
            return await _subtitleCueRewriter.RewriteAsync(
                itemId,
                format,
                original,
                startPositionTicks,
