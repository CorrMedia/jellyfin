...
- **Hardware encode (graph):** Consumer HW (NVENC/QSV/VAAPI/…) can decode and encode, but not a portable overlay / punch-in / concat / timed-box graph (`overlay_cuda`, `vpp_qsv`, `scale_vaapi` are vendor-specific and still miss concat and most corr.json filters). Path is HW decode (when known) + `hwdownload` + CPU filters + `GetVideoEncoder`. Mute-only still uses the full stock HW `-vf` path. Overlay/concat stay CPU-heavy; HDR/10-bit through the graph is flattened to 8-bit. Apply-edits off remains the escape hatch for titles that only play via HW transcode.
